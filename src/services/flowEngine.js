...
// Para un número normal, Meta espera { to: "51987654321" }. Para un
// BSUID, en cambio, espera { recipient: "PE.xxx", recipient_type:
// "individual" } — un campo distinto, no "to". (Probamos antes con
// recipient_type: "business_scoped_user_id", pero Meta lo rechaza —
// el valor correcto sigue siendo "individual", solo cambia el nombre
// del campo que lleva el identificador.)
function destinatarioFields(to) {
  return esBsuid(to)
    ? { recipient: to, recipient_type: 'individual' }
    : { to };
}
...
async function sendWhatsAppMessage(phoneNumberId, accessToken, to, message, conversationId) {
  ...
  try {
    await axios.post(
      `https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}/messages`,
      { messaging_product: 'whatsapp', ...destinatarioFields(to), type: 'text', text: { body: message } },
      { headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' } }
    );
...
async function sendWhatsAppButtons(phoneNumberId, accessToken, to, bodyText, buttons, conversationId, header = null, footerText = null) {
  try {
    ...
    await axios.post(
      `https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}/messages`,
      { messaging_product: 'whatsapp', ...destinatarioFields(to), type: 'interactive', interactive },
      { headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' } }
    );
    if (conversationId) {
...
async function sendWhatsAppImage(phoneNumberId, accessToken, to, url, caption, conversationId) {
  ...
  try {
    await axios.post(
      `https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}/messages`,
      { messaging_product: 'whatsapp', ...destinatarioFields(to), type: 'image', image: { link: url, caption: caption || '' } },
      { headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' } }
    );
...
async function sendWhatsAppVideo(phoneNumberId, accessToken, to, url, caption, conversationId) {
  ...
  try {
    await axios.post(
      `https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}/messages`,
      { messaging_product: 'whatsapp', ...destinatarioFields(to), type: 'video', video: { link: url, caption: caption || '' } },
      { headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' } }
    );
...
async function sendWhatsAppAudio(phoneNumberId, accessToken, to, url, conversationId) {
  ...
  try {
    await axios.post(
      `https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}/messages`,
      { messaging_product: 'whatsapp', ...destinatarioFields(to), type: 'audio', audio: { link: url } },
      { headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' } }
    );
...
async function sendWhatsAppDocument(phoneNumberId, accessToken, to, url, fileName, conversationId) {
  ...
  try {
    ...
    await axios.post(
      `https://graph.facebook.com/${GRAPH_VERSION}/${phoneNumberId}/messages`,
      { messaging_product: 'whatsapp', ...destinatarioFields(to), type: 'document', document: { link: url, filename: nombre } },
      { headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' } }
    );
...
async function sendPurchaseEventToMeta(userId, conversation, saleAmount) {
  try {
    // Primero intenta con la integración real "Meta CAPI Cloud" (un
    // Dataset distinto por cada WhatsApp conectado) — la que el
    // usuario configuró de verdad en Integraciones.
    ...
    // Respaldo: la configuración vieja de un solo pixel_id general,
    // por si el usuario todavía la tiene así en vez de la nueva.
    if (!datasetId || !accessToken) {
      ...
      if (!config || !config.conversions_api || !config.pixel_id || !config.access_token) {
        console.log(`[Meta Conversions API] Sin integración configurada para user ${userId} — no se envía nada.`);
        return;
      }
    ...
    // ctwa_clid van DENTRO de user_data, y el monto/moneda van DENTRO
    // de custom_data — no sueltos en el evento.
