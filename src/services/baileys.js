...
// ── Anti-Baneo: si el item tiene variaciones generadas por IA
// guardadas (item.antiBaneo === true y item.variaciones: [...]),
// elige una al azar en vez de mandar siempre el texto original —
// así no se manda el mismo mensaje idéntico a todo el mundo, uno de
// los patrones que WhatsApp usa para detectar bots.
...
async function saveMsg(conversationId, content, direction, msgType = 'text', mediaUrl = null) {
  ...
  try {
    ...
    // "Sin Respuesta" del Chat en Vivo.
    const updateData = {
    ...
    // "ever_replied" es distinto de "last_message_direction" — una vez
    // que el cliente respondió de VERDAD (después de que el bot ya le
    // había escrito antes), esto se queda en true PARA SIEMPRE, aunque
    // más adelante se le mande un seguimiento y no conteste. Así
    // "Respondieron" no se le "resetea" a un cliente que ya demostró
    // interés real, solo porque el bot le volvió a escribir.
...
async function callAIProvider(provider, apiKey, model, systemPrompt, conversationMessages, maxTokens = 500) {
  ...
  if (prov === 'gemini' || prov === 'google') {
    // Gemini usa un formato de request bien distinto a OpenAI/Groq:
    // "contents" en vez de "messages", roles 'user'/'model' en vez de
    // 'user'/'assistant', y el system prompt va aparte en
    // "systemInstruction", no como un mensaje más.
...
async function respondWithAIBaileys(userId, sock, jid, userMessage, conversationId, aiConfigIdOverride = null, nodePrompt = null) {
  ...
  try {
    ...
    if (!aiResponse || !aiResponse.trim()) {
      // Se quedó sin nada útil después de limpiar el razonamiento —
      // mejor decir algo genérico que quedarse en silencio total,
      // que para el cliente se ve como que el bot lo ignoró.
      console.warn('[Baileys AI] La respuesta quedó vacía después de limpiar el razonamiento — usando mensaje de respaldo');
      await sendText(sock, jid, 'Dame un momento para revisar eso 🙏', conversationId);
      return;
    }
      ...
      case 'content': {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          if (tipo === 'text' || tipo === 'texto') {
            ...
            await sendImage(sock, jid, item.url || '', elegirTextoConVariacion(item, captionBase), conversationId);
            textoNodoCompleto += ' ' + captionBase;
          } else if (tipo === 'video') {
            const captionBase = item.caption || item.description || '';
            await sendVideoMsg(sock, jid, item.url || '', elegirTextoConVariacion(item, captionBase), conversationId);
            textoNodoCompleto += ' ' + captionBase;
          } else if (tipo === 'audio') {
...
async function callVisionAI(provider, apiKey, promptText, base64Image, mimeType, maxTokens = 400) {
  ...
  // Groq y OpenAI comparten formato ("OpenAI-compatible")
  const baseUrl = prov === 'openai'
    ? 'https://api.openai.com/v1/chat/completions'
    : 'https://api.groq.com/openai/v1/chat/completions';
